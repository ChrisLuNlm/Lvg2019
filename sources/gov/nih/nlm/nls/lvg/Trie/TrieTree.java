...
public class TrieTree
{
    ...
    */
        ...
        boolean duplicateFlag)
    {
        ...
        LoadRulesFromFile(inFile, reverse, duplicateFlag);
    }
    /**
    * Read in LVG trie rules from a specified rule.
    *
    * @param   inFile   the full path of the LVG trie rule file  
    * @param   reverse   true or false to load the rule in forward or backward
    * @param   duplicateFlag   flag to send warning message when duplicate rule
    * direction from the file.  All rules in LVG trie are bi-directional.
    */
    public void LoadRulesFromFile(String inFile, boolean reverse,
        boolean duplicateFlag)
    {
        try
        {
            ...
            while((line = in.readLine()) != null)
            {
                ...
                {
                    if(line.startsWith(EXCEPTION_STR) == true) // An Exception
                    {
                        
                        String exception 
                            = line.substring(EXCEPTION_STR.length());
                        line = ReverseException(exception, reverse);
                        curRule_.AddException(line);
                        exceptionNum_++;
                    }
                    else if(line.startsWith(RULE_STR) == true)        // a Rule
                    {
                    ...
                    else if (line.startsWith(FILE_STR) == true)   // file
                    {
                        StringTokenizer buf = new StringTokenizer(line, " ");
                        buf.nextToken();
                        ...
                        LoadRulesFromFile(newInFile, reverse, duplicateFlag);
                    }
                }
            }
            in.close();
    ...
    *
    * @return A vecotr of TrieNode with rule matches the inStr
    */
