...
public class ToFruitfulVariants extends Transformation implements Cloneable
{
    ...
    * of LexItems
    *
    * @throws SQLException for DB exceptions
    *
    * @see DbBase
    */
    ...
    */
        ...
        Vector<LexItem> newList)
    {
        ...
        for(int i = 0; i < newList.size(); i++)
        {
            ...
            if(ht.containsKey(curNewKey) == true)
            {
                ...
                // new features in 2018, use shortest length if distance is same
                else if (existDist == curNewDist)
                {
                    // use the shorter length of history
                    if(existRec.GetFlowHistory().length() 
                        > curNew.GetFlowHistory().length())
                    {
                        ht.remove(curNewKey);
                        ht.put(curNewKey, curNew);
                    }
        ...
        Connection conn, RamTrie trieD, boolean detailsFlag)
    {
        ...
        // get derivations
        // 2018: Only use not-negative suffixD for recursive derivations
        // prefixD and zeroD do not contribute too much recall
        for(int i = 0; i < tempIns.size(); i++)
        {
            LexItem curIn = tempIns.elementAt(i);
            Vector<LexItem> derivations = 
                ToRecursiveDerivations.Mutate(curIn, conn, trieD, 
                OutputFilter.LVG_ONLY, 
                OutputFilter.D_TYPE_SUFFIX, OutputFilter.D_NEGATION_OTHERWISE,
        ...
        Connection conn, boolean detailsFlag)
    {
        ...
        {
            LexItem curIn = tempIns.elementAt(i);
            int srcOption = OutputFilter.S_SRC_CUI_NLP;
            Vector<LexItem> rSynonyms = ToRecursiveSynonyms.Mutate(curIn, conn, 
                srcOption, detailsFlag, false, true);
    ...
    // data members
    private final static String INFO = "Generate Fruitful Variants";
}
