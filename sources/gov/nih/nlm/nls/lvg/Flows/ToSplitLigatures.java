...
public class ToSplitLigatures extends Transformation implements Cloneable
{
    ...
    */
        ...
        Hashtable<Character, String> ligatureMap)
    {
        ...
        else // use unicode normalization NFKC algorithm
        {
            ...
            outStr = Normalizer2.getNFKCInstance().normalize(inCharStr);
        }
    ...
    */
        ...
        boolean detailsFlag, boolean mutateFlag)
    {
        ...
        for(int i = 0; i < inStr.length(); i++)
        {
            ...
            else    // use unicode normalization NFKC algorithm
            {
                ...
                String normStr = Normalizer2.getNFKCInstance().normalize(
                    curCharStr);
                // remove space character from both ends
                if(normStr.length() > 1)
                {
    ...
    // data members
    private final static String INFO = "Split Ligatures";
    private final static String NO_OPERATION = "NO";
    private final static String MAPPING = "MP";
    private final static String NORM_NFKC = "NFKC";
}
